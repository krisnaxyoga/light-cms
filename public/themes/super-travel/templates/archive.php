...
<section class="st-portfolio st-portfolio--archive">
    <div class="st-section-inner">
        <h1 class="st-portfolio__heading st-reveal"><?= esc($archiveTitle ?? 'Archive') ?></h1>
        <?php if (! empty($archiveIntro)): ?>
            <p class="st-portfolio__intro st-reveal"><?= esc($archiveIntro) ?></p>
        <?php endif; ?>

        <?php if (empty($posts)): ?>
            <p class="st-empty">No posts here yet.</p>
        <?php else: ?>
